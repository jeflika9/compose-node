...
impl Path {
	...
	pub fn new_from_vec(path: Vec<u8>) -> Result<Path, SerString> {
		Self::new_from_path(SerString::from_vec(path))
	}
}
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {	
		...
		/// Return Some(registered path) if registered, None if not registered, or MalformedPath if the given domain and route are invalid
		pub fn get_registered_path_info_from_vec(path_vec: Vec<u8>) -> Option<RegisteredPath<T>> {
			match Path::new_from_vec(path_vec) {
				Ok(path) => Pallet::<T>::get_registered_path_info(&path),
				_ => None,
			}
		}
